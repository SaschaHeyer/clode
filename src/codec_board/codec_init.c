...
{
    // NOTE: the IDF>=5.4 path here originally called i2c_master_get_bus_handle()
    // to look the bus up from the driver's own registry instead of using the
    // handle _i2c_init() already stored below. On IDF 5.5.4 (this board's core)
    // that lookup does not resolve the bus that was just created, and the codec
    // then fails I2C reads/writes with "this port has not been initialized".
    // The local table is always populated correctly by _i2c_init(), so just use it.
